...
def getmessage2(url):
    ...
    try:
        ...
        with open(filename, 'ab') as f:
            ...
            f.write(result[0].encode())
            f.write('  ，  '.encode())
            pattern = re.compile('<span class="up">(.*?)m²</span>', re.S)  # 5000m²
            result = pattern.findall(res.text)
            print(result)
            f.write(result[0].encode())
            f.write('  ，  '.encode())
            pattern = re.compile('<span class="house_basic_title_money_num_second">(.*?)&nbsp;&nbsp;&nbsp;&nbsp;</span>', re.S)  # 0.9
            result = pattern.findall(res.text)
...
if __name__ == "__main__":
        ...
        url = 'http://xm.ganji.com/fangchan/44471229437985x.shtml?prd=WKoNq50yyuEYSNK3tGzyEw%3D%3D&houseId=1770275683999749'#厂房
