...
def downloading(workQueue,j):
    while not workQueue.empty():
        ...
        try:  # 捕获异常
            ...
            print('第%s次·第一次网络状态码：'%y,res.status_code)
            ...
            for url in list1:
                res = requests.get(url, headers=header, proxies=proxy, timeout=10)
                print('第%s次·第二网络状态码：' % y, res.status_code)
...
if __name__ == '__main__':
    ...
    for page in range(1, 100):
        workQueue.put('https://search.bilibili.com/article?keyword=%E5%9B%BE%E7%89%87&page={}'.format(page))

    pool=Pool(processes=10)
    for i in range(10):
        pool.apply(downloading,args=(workQueue,i))

    print("进程开始")
    pool.close()#当进程池close的时候并未关闭进程池，只是会把状态改为不可再插入元素的状态，完全关闭进程池使用
    pool.join()
