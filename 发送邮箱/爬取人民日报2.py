...
def sendmail(result):
    ...
    msgText = MIMEText(
        ...
        + '</font><' + '<br><img src="cid:image1"><br>' + news + '</body></html>', 'html', 'utf-8')
...
def getlist1(url='https://search.bilibili.com/article?keyword=p%E7%AB%99'):
    ...
    try:
        ...
        header = {'User-Agent': random.choice(user_agent_list)}  # 随机选一个user-agent
        res = requests.get(url, headers=header, timeout=20)
        print('准备下载图片getlist1·第%s轮·第一次网络状态码: ' % j, res.status_code)
        # print(res.content.decode('utf-8'))
...
def getlist2(url):
    ...
    print('下载图片·getlist2第%s轮·第二网络状态码：' % i, res.status_code)
...
def getlist3(url):
    ...
    try:
        ...
        # print(res.content)
        # pattern = re.compile('try \{ window.getTimelineService1 = (.*?)"infoSource":"央视新闻app"', re.S)   # 综合得分
        # result = pattern.findall(res.content.decode('utf-8'))
        # print(result[-1:])
        pattern2 = re.compile(',"pubDateStr":"(.*?),"infoSource":"央视新闻app","sourceUrl":', re.S)  # 综合得分
        result2 = pattern2.findall(res.content.decode('utf-8'))
        # print(''.join(result2))
        a = (str(''.join(result2))).replace('\\n', '')  # 替换\n
        # print(a)
        # print((''.join(result2)))

        filename = r'C:\\Users\\Administrator\\Desktop\\新建文件夹/'
        with open(filename + 'abc.txt', 'w', encoding='utf-8') as fp:
...
if __name__ == "__main__":
    schedule.every().day.at("19:55").do(getlist1)
    getlist1()
    ...
    schedule.every().day.at("19:56").do(getlist, url)
    getlist(url)
    while True:
